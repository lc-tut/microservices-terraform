...
run "private_vm" {
  command = plan
  assert {
    condition     = toset(keys(openstack_compute_instance_v2.vm)) == toset(["billing"]) && openstack_compute_instance_v2.vm["billing"].name == "lcn-billing-api"
    error_message = "この root は lcn-billing-api 1台だけを管理します。"
  }
  assert {
  ...
  assert {
    condition     = yamldecode(trimprefix(openstack_compute_instance_v2.vm["billing"].user_data, "#cloud-config\n")).ssh_pwauth == false
...
run "explicit_ingress_and_floating_ip" {
  ...
  assert {
    condition = length(openstack_networking_secgroup_rule_v2.ingress) == 3 && alltrue([
      for rule in openstack_networking_secgroup_rule_v2.ingress :
